<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
	<h1>상품 수정 페이지</h1>
	<br>
	<form action="edit.pd" method = "post">
	
		<%-- disabled 된 정보는 가져오지 못해서 hidden으로 input으로 pvo.pno를 받는걸 하나 더 만들어준다. --%>
		<input type="hidden" name="pno" value="${pvo.pno }">
		상품번호 : <input type="text" name ="pno" value="${pvo.pno }" disabled="disabled"><br>
		상품이름 : <input type="text" name="pname" value="${pvo.pname }"><br>
		상품가격 : <input type="text" name="price" value="${pvo.price }"><br>
		등록일자 : <input type="text" name ="pno" value="${pvo.regdate }" disabled="disabled"><br>
		세부내용 : <input type="text" name="madeby" value="${pvo.madeby }"><br>

	<button type="submit">수정완료</button>
	</form>
</body>
</html>